...
The committed vhost is the post-certbot config for the live host, so it
references a cert under `/etc/letsencrypt/` that does not exist on a fresh
host, and nginx will refuse to load it there. For a rebuild, bring TLS up in
order:

1. **HTTP only (Ansible).** Temporarily deploy the vhost without its
   `# managed by Certbot` lines (a single `listen 80` server proxying to the app
   on loopback) — enough for the http-01 challenge.
